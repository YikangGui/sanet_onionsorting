...
def keras_version():
    ...
    """
    return tuple(map(int, tensorflow.keras.__version__.split('.')))
...
def assert_keras_version():
    ...
    """
    detected = tensorflow.keras.__version__
    required = '.'.join(map(str, minimum_keras_version))
    assert(keras_version() >= minimum_keras_version), 'You are using tensorflow.keras version {}. The minimum required version is {}.'.format(detected, required)
