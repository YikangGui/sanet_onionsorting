...
def default_classification_model(
    ...
    if tensorflow.keras.backend.image_data_format() == 'channels_first':
        inputs  = tensorflow.keras.layers.Input(shape=(pyramid_feature_size, None, None))
    else:
        inputs  = tensorflow.keras.layers.Input(shape=(None, None, pyramid_feature_size))
    outputs = inputs
    for i in range(4):
        outputs = tensorflow.keras.layers.Conv2D(
            ...
            kernel_initializer=tensorflow.keras.initializers.normal(mean=0.0, stddev=0.01, seed=None),
    ...
    if tensorflow.keras.backend.image_data_format() == 'channels_first':
        outputs = tensorflow.keras.layers.Permute((2, 3, 1), name='pyramid_classification_permute')(outputs)
    outputs = tensorflow.keras.layers.Reshape((-1, num_classes), name='pyramid_classification_reshape')(outputs)
    outputs = tensorflow.keras.layers.Activation('sigmoid', name='pyramid_classification_sigmoid')(outputs)
...
def default_regression_model(num_values, num_anchors, pyramid_feature_size=256, regression_feature_size=256, name='regression_submodel'):
    ...
    outputs = tensorflow.keras.layers.Conv2D(num_anchors * num_values, name='pyramid_regression', **options)(outputs)
    if tensorflow.keras.backend.image_data_format() == 'channels_first':
        outputs = tensorflow.keras.layers.Permute((2, 3, 1), name='pyramid_regression_permute')(outputs)
    outputs = tensorflow.keras.layers.Reshape((-1, num_values), name='pyramid_regression_reshape')(outputs)
...
def __create_pyramid_features(C3, C4, C5, feature_size=256):
    ...
    P4           = tensorflow.keras.layers.Add(name='P4_merged')([P5_upsampled, P4])
    ...
    P3 = tensorflow.keras.layers.Add(name='P3_merged')([P4_upsampled, P3])
...
def __build_model_pyramid(name, model, features):
    ...
    """
    return tensorflow.keras.layers.Concatenate(axis=1, name=name)([model(f) for f in features])
...
def __build_anchors(anchor_parameters, features):
    ...
    return tensorflow.keras.layers.Concatenate(axis=1, name='anchors')(anchors)
