...
def focal(alpha=0.25, gamma=2.0):
    ...
    def _focal(y_true, y_pred):
        ...
        alpha_factor = backend.where(tensorflow.keras.backend.equal(labels, 1), alpha_factor, 1 - alpha_factor)
        focal_weight = backend.where(tensorflow.keras.backend.equal(labels, 1), 1 - classification, classification)
        ...
        return tensorflow.keras.backend.sum(cls_loss) / normalizer
...
def smooth_l1(sigma=3.0):
    ...
    def _smooth_l1(y_true, y_pred):
        ...
        # filter out "ignore" anchors
        indices           = backend.where(tensorflow.keras.backend.equal(anchor_state, 1))
        ...
        return tensorflow.keras.backend.sum(regression_loss) / normalizer
